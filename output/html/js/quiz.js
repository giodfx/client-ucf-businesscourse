...
(function () {
  ...
  function checkAnswer(questionIndex) {
    ...
    if (window.scormAPI) recordInteraction(question, result);
  }
  ...
  function recordInteraction(question, result) {
    ...
    try {
      if (window.scormAPI && typeof window.scormAPI.recordInteraction === 'function') {
  ...
  function submitQuiz() {
    ...
    if (window.scormAPI && typeof window.scormAPI.setScore === 'function') {
      try {
        window.scormAPI.setScore(percentage, 100, 0);
        window.scormAPI.setStatus(passed ? 'passed' : 'failed');
      } catch (e) { /* ignore */ }
    }
  }
